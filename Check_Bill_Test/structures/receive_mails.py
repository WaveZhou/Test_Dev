...
def get_parsed_msg(useraccount, password, limits=1, debuglevel=1):
    ...
    for item in mails[-1:-(limits + 1):-1]:
        ...
        mailinfo = Mail.MailInfo(index, size)
...
def get_rawcontent_charset(rawcontent):
    for item in rawcontent:
        if str(decode_byte(item)).__contains__('charset='):
            #charset = [re.findall(re.search('charset="(.+)"'), decode_byte(item))]
            charset = re.search(r'charset=(.+)',decode_byte(item)).group(1)
            return charset
            # charset_list = list()
...
if __name__ == '__main__':
    # input('Mail User Account:')
    useraccount = 'gzzy@jiumingfunds.com'
    # getpass.getpass(prompt='Your Auth Code:')
    password = 'Guzhi123'
    # int(input('How many mails you want to receive:'))
    limits = 10
    # int(input('the debug level, default is on which is number 1'))
    debug_level = 5
    ...
    while not exitcode:
        ...
        print("[" + str(
            index) + ": " + maildetails.subject + ", " + maildetails.from_nickname + ", " + maildetails.receivedtime + ", " + maildetails.text_content + "]")
        # 询问是否退出邮件查询系统，是则退出，否则继续进行下一步的查询操作
        userinput = input('ready to exit? (Y/N)')
        exitcode = 0 if (userinput == 'y' or userinput == 'Y') else 1

mails = get_parsed_msg('gzzy@jiumingfunds.com', 'Guzhi123', 3, 1)
# data = mails[1].data
